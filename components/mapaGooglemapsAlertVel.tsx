...
async function createMarkerIcon(markerIcon: string | any, rotation: number = 0): Promise<any> {
  try {
    // Extrae la URL si el parámetro es un objeto
    const iconUrl = typeof markerIcon === 'string' ? markerIcon : markerIcon?.url || markerIcon;
    
    // Solo procesar archivos SVG que contengan 'streamingSemaforos'
    if (typeof iconUrl === 'string' && iconUrl.includes('streamingSemaforos.svg')) {
...
interface MapaGoogleMapsProps {
  ...
  onChangeActualizar: (on: boolean, lati: number, longi: number, zoom: number) => void;
...
export default function MapaGoogleMapsAlertVel({ ubicaciones,velocidadesWaze1,velocidadesWaze2,velocidadesWaze3,velocidadesWaze4,velocidadesWaze5, tramoPMT, desvios, inteligentes, opcDropdownVel, lat, lng, congestionDataAlta, congestionDataMedia, congestionDataBaja, ubicacionesAlertas, onChangeActualizar, onChangeActualizarStreaming, onChangeOrigins, onChangeRecomendaciones, zoom, recomendaciones, areaslocalidades }: MapaGoogleMapsProps) {
  ...
  const [destinations, setDestinations] = useState<google.maps.LatLngLiteral>({lat: 0, lng: 0});
  ...
  const recommendationPolylinesDataRef = useRef<Array<{
  ...
  const getIconUrl = (fuente: string, incidente: string, valorLectura: number) => {
    if (
    ...
    else if ( (fuente === 'WAZE' || fuente === 'IRREGULARIDADES') && (
      ...
      incidente === 'ACCIDENT_MINOR' ||
      incidente === 'ACCIDENT_MAJOR' )
    ) {
    ...
    else if (fuente === 'STREAMING'  && (
      incidente === 'Activa' )) { 
      return iconUrlStreaming;
    } 
    else if (fuente === 'STREAMING'  && (
      incidente === 'No Funciona' )) { 
    ...
    else if (fuente === 'LLUVIAS'  && incidente === 'Sin Lluvias' && valorLectura === 0) { 
      return iconUrlLluvias0;
    }
    else if (fuente === 'LLUVIAS'  && incidente === 'Sin Lluvias' && valorLectura >= 0.1) { 
      return iconUrlLluvias0Gif;
    }
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Bajos' && valorLectura === 0) { 
      return iconUrlLluvias1;
    } 
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Bajos' && valorLectura >= 0.1) { 
      return iconUrlLluvias1Gif;
    } 
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Moderados' && valorLectura === 0) { 
      return iconUrlLluvias2;
    }
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Moderados' && valorLectura >= 0.1) { 
      return iconUrlLluvias2Gif;
    } 
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Altos' && valorLectura === 0) { 
      return iconUrlLluvias3;
    }
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Altos' && valorLectura >= 0.1) { 
      return iconUrlLluvias3Gif;
    } 
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Muy Altos' && valorLectura === 0) { 
      return iconUrlLluvias4;
    }
    else if (fuente === 'LLUVIAS'  && incidente === 'Acumulados Muy Altos' && valorLectura >= 0.1) { 
  ...
  useEffect(() => {
    if (savedBoundsRef.current && mapInstanceRef.current && recomendaciones === 0) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      let zoomMapa = 10;      
      if((lat === 0  && lng === 0) || (lat === 4.6573506 && lng === -74.0909468))
      {
        lat = 4.6573506;
        lng = -74.0909468;
        zoomMapa = 12.5;
      ...
      autocomplete.setBounds(map.getBounds() as google.maps.LatLngBounds);      
        ...
        console.warn('Google Maps Places API no está disponible o el elemento pac-input no se encontró');
      }
      ...
      if(destinations && destinations.lat != 0 && destinations.lng !=0)
      {
        ...
        var marcador: google.maps.Marker = new window.google.maps.Marker({
          position: destinations as google.maps.LatLng | google.maps.LatLngLiteral,
          map: map,
      ...
      if(recomendaciones == 1)
      {
          google.maps.event.addListener(map, "click", (event) => {
            ...
            var marcador: google.maps.Marker = new window.google.maps.Marker({
              position: event.latLng as google.maps.LatLng | google.maps.LatLngLiteral,
              map: map,
              icon: iconRecomendacionesOrigen,
            });
      ...
      {
        loadGeoJsonDataTramoPMT(makeGeoJSONVel(tramoPMT,"pmt"), map, '#600924', infoWindow);
      }  

      if(desvios && desvios.length > 0)
      {
        loadGeoJsonDataDesvios(makeGeoJSONVel(desvios,"desvios"), map, '#7045ec', infoWindow);
      }      

      if(inteligentes && inteligentes.length > 0)
      {
        loadGeoJsonDataInteligentes(makeGeoJSONVel(inteligentes,"inteligentes"), map, '#ec4570', infoWindow);
      }
      
      if(ubicacionesAlertas !== null && ubicacionesAlertas !== undefined && ubicacionesAlertas.length > 0)
      {
        const markersAlertas = ubicacionesAlertas.map((ubicacionAlertas) => {
        ...
        const clustererAlertas = new MarkerClusterer({ markers: markersAlertas, map });

      }

      // ========== KMZ LAYER PARA NUBES DE RADAR ==========
      const hasLluviasData = ubicaciones.some(ubicacion => ubicacion[11] === 'LLUVIAS');
      ...
      if(areaslocalidades && areaslocalidades.length > 0 && ubicaciones.length > 0 && hasLluviasData)
      {
        console.log("Areas localidades:", areaslocalidades);

        const colors = ['#c700385b', '#ff58334b', '#900c3f3d', '#5818456c', '#1f618d4f', '#117a6550', '#27ae5f46', '#f39d123f', '#d4ac0d44', '#f7dc6f63', '#e74d3c41', '#8d44ad52', '#3498db56', '#16a0845b', '#2c3e506c'];
      ...
      type CategoryKey = 'incidente' | 'waze' | 'irregularidades' | 'pmt' | 'streaming' | 'lluvias' | 'salvavidas' | 'camarasCGT' | 'semaforos' | 'inteligentes';
      ...
      if(ubicaciones !== null && ubicaciones !== undefined && ubicaciones.length > 0)
      {
        ...
        const createSingleMarker = async (item: any) => {
          ...
          const rotacion = fuente === 'STREAMINGSEMAFOROS' ? parseFloat(item[14]) || 0 : 0;
          ...
          {
            iconData = await createMarkerIcon(getIconUrl(fuente, incidente, 0), rotacion);
          } 
          else 
          {
        ...
        const createMarkers = async () => {
          for (const item of ubicaciones) {
            ...
            if (result) {
              ...
              if (fuente === 'SEMAFOROS' || fuente === 'SEMAFOROSNO' || fuente === 'SEMAFOROSFALLA') 
              {
              ...
              else if (fuente === 'STREAMING' || fuente === 'STREAMINGSEMAFOROS')  
              {
        ...
        Object.entries(markersByCategory).forEach(([category, markers]) => {
          if (markers.length > 0) 
          {
            clustererRefs.current[category as CategoryKey] = new MarkerClusterer({
              map,
              markers,
              renderer: {
      ...
      else if(opcDropdownVel === 'JAMS' || opcDropdownVel === 'oprimirJAMS')
      {
        loadGeoJsonData(makeGeoJSONVel(velocidadesWaze1,"vel"), map, '#20ac54', infoWindow);
        loadGeoJsonData(makeGeoJSONVel(velocidadesWaze2,"vel"), map, '#f4c30d', infoWindow);
        loadGeoJsonData(makeGeoJSONVel(velocidadesWaze3,"vel"), map, '#f6834a',infoWindow);
        loadGeoJsonData(makeGeoJSONVel(velocidadesWaze4,"vel"), map, '#c10408',infoWindow);
        loadGeoJsonData(makeGeoJSONVel(velocidadesWaze5,"vel"), map, '#6d021d',infoWindow);
      }
      else if(opcDropdownVel === 'CONGESTION' || opcDropdownVel === 'oprimirCONGESTION')
      {
        loadGeoJsonData(makeGeoJSONVel(congestionDataAlta,"vel"), map, '#6bca70', infoWindow);
        loadGeoJsonData(makeGeoJSONVel(congestionDataMedia,"vel"), map, '#f19d5d', infoWindow);
        loadGeoJsonData(makeGeoJSONVel(congestionDataBaja,"vel"), map, '#dd4e47',infoWindow);
      }
      ...
      if (recommendationPolylinesDataRef.current.length > 0) {
        console.log('Redibujando polilíneas de recomendaciones en el nuevo mapa...');

        recommendationPolylinesDataRef.current.forEach(polylineData => {
          const { polyline, originAddress, destinationAddress, tiempo, distancia } = polylineData;
    ...
    const moduloRecomendaciones = (destinationCoords?: google.maps.LatLngLiteral, infoWindow?: any) => {
      ...
      if(origins.length > 0 && (finalDestination.lat !== 0 && finalDestination.lng !==0))
      {
        
          const originLatLngs = origins.map(origin => new google.maps.LatLng(origin.lat, origin.lng));
          const destinationLatLng = new google.maps.LatLng(finalDestination.lat, finalDestination.lng); 
            ...
            (response, status) => {
              ...
              else 
              {
                ...
                if (rutasConTiempo.length > 0) 
                {
                  ...
                  calculateAndDisplayRoute(directionsService, map, mejorRuta.origin, finalDestination, infoWindow, mejorRuta.addressText, destinationAddress, mejorRuta.durationText, mejorRuta.distanceText);
                }
    ...
    const calculateAndDisplayRoute = (
      ...
      const selectedMode: google.maps.TravelMode = google.maps.TravelMode.DRIVING;

      if((originCoords.lat === 0 && originCoords.lng ===0) || (destinationCoords.lat === 0 && destinationCoords.lng ===0))
      {
        ...
        directionsService.route(
          ...
          (response: google.maps.DirectionsResult | null, status: google.maps.DirectionsStatus) => {
            if (status === 'OK' && response) {
              ...
              const destinationAddressFixed = destinationAddress.split(', Bogotá')[0];

              //dibuja la polilinea de la ruta recomendada
              drawPolyline(routePath, map, infoWindow, originAddressFixed, destinationAddressFixed, tiempo, distancia, destinationCoords);
            } 
            else 
            {
    ...
    const drawPolyline  = (routePath: google.maps.LatLng[], map: google.maps.Map, infoWindow: any, originAddress: string, destinationAddress: string, tiempo: string, distancia: string, destinationCoords: google.maps.LatLngLiteral) => {
    ...
    const makeGeoJSONVel = (velocidadesData: any, info: string) => {
     // console.log(velocidadesData);
      ...
      return {
        type: 'FeatureCollection',
        features: velocidadesData.filter((velDatos: any) => velDatos !== null) // Filtrar elementos nulos
        .map((velDatos: any) => {
    ...
    const loadGeoJsonDataTramoPMT= (geoJsonData: any, map: any, strokeColor: string, infoWindow: any) => {
      ...
      dataLayer.addListener('click', (event: any) => {
        ...
        featuresDiv.appendChild(createFeature(iconUrlLocation, "Velocidad", localidad));
    ...
    const loadGeoJsonDataInteligentes = (geoJsonData: any, map: any, strokeColor: string, infoWindow: any) => {
        ...
        geoJsonData.features.forEach((feature: any) => {
              ...
              Array.isArray(feature.geometry.coordinates))    
          {
            ...
            const pathCoordinates = feature.geometry.coordinates.map((coord: [number, number]) => ({
              lat: coord[1],
              lng: coord[0]
            }));
            ...
            const polyline = new google.maps.Polyline({
              ...
              geodesic: true,
              strokeColor: feature.properties.nivel_alerta < 10 ? "#FF0000" : feature.properties.nivel_alerta > 20 ? "#760D06" : "#E3615B",
              strokeOpacity: 0.8,
              strokeWeight: 10,
            });
    ...
    const loadGeoJsonDataDesvios = (geoJsonData: any, map: any, strokeColor: string, infoWindow: any) => {
        ...
        geoJsonData.features.forEach((feature: any) => {
              ...
              Array.isArray(feature.geometry.coordinates))    
          {
            ...
            const intervalId = window.setInterval(animateIcon, velocidadAnimacion);
            animationRefs.current.push(intervalId);           

            google.maps.event.addListener(animatedLine, 'click', (event: google.maps.PolyMouseEvent) => {
    ...
    const loadGeoJsonData = (geoJsonData: any, map: any, strokeColor: string, infoWindow: any) => {
    ...
    const buildContentInteligentes = (id: number) => {
      ...
      else
      {
        ...
        const afectados = ubicacion.afectados !== undefined && ubicacion.afectados !== null
        ? Number.parseFloat(ubicacion.afectados).toFixed(2)
        : "0.00";
        const nivel_alerta  = ubicacion.nivel_alerta;
        const trend  = ubicacion.trend === 1 ? "Empeorando" : ubicacion.trend === -1 ? "Mejorando" : "Igual";
        ...
        const estado_semaforo = ubicacion.estado_semaforo === null || ubicacion.estado_semaforo === undefined ? "Sin información" : ubicacion.estado_semaforo;
        ...
        const tipos_alertas_waze = ubicacion.tipos_alertas_waze === null || ubicacion.tipos_alertas_waze === undefined ? "Sin información" : ubicacion.tipos_alertas_waze;
        const id_incidente = ubicacion.id_incidente === null || ubicacion.id_incidente === undefined ? "Sin información" : ubicacion.id_incidente;
        const tipo_incidente = ubicacion.tipo_incidente === null || ubicacion.tipo_incidente === undefined ? "Sin información" : ubicacion.tipo_incidente;

        const indice_congestion = ubicacion.indice_congestion !== undefined && ubicacion.indice_congestion !== null
        ...
        if(estado_mvi)
        {
          estado_mvi.split(" | ").forEach((word: string, index: number) => {
            ...
            if (word === "POBRE" || word === "MUY POBRE" || word === "FALLADO" || word === "GRAVE") {
        ...
        appendFeatureIC(iconIndiceCongestion, "Indice de congestión", indice_congestion);
        ...
        const appendFeatureTipoCongestion = (src: string, title: string, text: string) => {
          const feature = document.createElement("div");
        ...
        appendFeatureTipoCongestion(iconUrlCongestion2, "Tipo de congestión", tipo_congestion);
        ...
        const appendFeatureAfectados = (src: string, title: string, text: string) => {
          const feature = document.createElement("div");
        ...
        appendFeatureAfectados(iconAfectados, "Afectados", `${afectados} personas`);
        ...
        const appendFeatureTamanoCongestion = (src: string, title: string, text: string) => {
          const feature = document.createElement("div");
        ...
        appendFeatureTamanoCongestion(iconTamanoCongestion, "Tamaño de la congestión", `${length} metros`);
       ...
       // ==================== BLOQUE DE ALERTAS ====================
        ...
        const appendFeatureAlerta = (label: string, src: string, title: string, text: string) => {
        ...
        appendFeatureAlerta("Cantidad de alertas:", iconCantidadAlertasWaze, "Cantidad de alertas", cantidad_alertas_waze);
        appendFeatureAlerta("Tipo de alertas:", iconTiposAlertasWaze, "Tipos de alertas", tipos_alertas_waze);
        ...
        const appendFeatureIncidente = (label: string, src: string, title: string, text: string) => {
        ...
        appendFeatureIncidente("ID incidente:", iconIdIncidente, "ID incidente", id_incidente);
        appendFeatureIncidente("Tipo de incidentes:", iconTipoIncidente, "Tipos de incidentes", tipo_incidente);
        ...
        appendFeature(iconUrlClockCongestion, "Tiempo de congestión", `${tiempo_en_congestion} minutos`);
        ...
        const iconoAcumulado = acumuladodia_clima === 0 ? iconUrlAcumiladoDia1
          : acumuladodia_clima > 0 && acumuladodia_clima <= 10 ? iconUrlAcumiladoDia2
          : acumuladodia_clima > 10 && acumuladodia_clima <= 30 ? iconUrlAcumiladoDia3
          : acumuladodia_clima > 30 && acumuladodia_clima <= 50 ? iconUrlAcumiladoDia4
        ...
        appendFeature2(iconUrlSemaforoEstado, "Estado de semaforos", estado_semaforo);
        //appendFeature2(iconCoordenadas, "Coordenadas", `${coordinates[0]}`);

        if (coordinates[0]) 
        {
          appendFeature2(iconCoordenadas, "Coordenadas", `${coordinates[0][1]} , ${coordinates[0][0]}`);
        }
    ...
    const buildContent = (ubicacion: any) => {
      ...
      const iconImg = document.createElement("img");
      const iconUrl = fuente === 'WAZE' ? getIconUrl(fuente, descripcion, 0)
        : fuente === 'IRREGULARIDADES' ? getIconUrl(fuente, descripcion, 0)
        : fuente === 'INTELIGENTES' ? iconUrlInteligentes
        : fuente === 'P1' ? iconUrlWarning
        : fuente === 'CAMARASSALVAVIDAS' ? iconUrlCamsalvavidas
        : fuente === 'CAMARASCGT' ? iconUrlCamCGT
        : fuente === 'SEMAFOROS' ? iconUrlSemaforos
        : fuente === 'SEMAFOROSNO' ? iconUrlSemaforosNO
        : fuente === 'SEMAFOROSFALLA' ? iconUrlSemaforosFalla
        : fuente === 'STREAMING' ? iconUrlStreaming
        : fuente === 'STREAMINGSEMAFOROS' ? iconUrlStreamingSemaforos1
        : fuente === 'PMT' ? iconUrlPMT
        : fuente === 'CONGESTIONPRIORIDAD' ? iconUrlPrioridad
        : fuente === 'CONGESTIONPRIORIDADSEMAFOROS' ? iconUrlSemaforosNO
        : fuente === 'LLUVIAS' ? iconUrlLluvias
        : '';
    
      iconImg.src = typeof iconUrl === 'string' ? iconUrl : iconUrl?.url || '';
      iconImg.alt = "icon";
      iconImg.title = fuente === 'WAZE' ? 'Waze'
        : fuente === 'IRREGULARIDADES' ? 'Irregularidades'
        : fuente === 'INTELIGENTES' ? 'Inteligentes'
        : fuente === 'P1' ? 'Premiere One'
        : fuente === 'CAMARASSALVAVIDAS' ? 'Cámara salvavidas'
        : fuente === 'CAMARASCGT' ? 'Cámara CGT'
        : fuente === 'SEMAFOROS' ? 'Semaforización'
        : fuente === 'SEMAFOROSNO' ? 'Semaforización'
        : fuente === 'SEMAFOROSFALLA' ? 'Semaforización'
        : fuente === 'STREAMING' ? 'Streaming'
        : fuente === 'STREAMINGSEMAFOROS' ? 'Streaming Semáforos'
        : fuente === 'PMT' ? 'PMT'
        : fuente === 'CONGESTIONPRIORIDAD' ? 'Prioridad'
        : fuente === 'CONGESTIONPRIORIDADSEMAFOROS' ? 'Prioridad'
        : fuente === 'LLUVIAS' ? 'Lluvias'
        : '';
      ...
      const idDiv = document.createElement("div");
      idDiv.classList.add("id");
      idDiv.textContent = fuente === "CONGESTIONPRIORIDAD"  || fuente === 'CONGESTIONPRIORIDADSEMAFOROS' ? estado
        : fuente === "LLUVIAS" || fuente === "INTELIGENTES" || fuente === 'SEMAFOROS' || fuente === 'SEMAFOROSNO' || fuente === 'SEMAFOROSFALLA' || fuente === 'P1' ? direccion 
        : fuente === "STREAMINGSEMAFOROS" ? idCruceSemaforos 
        : id;
      detailsDiv.appendChild(idDiv);
      ...
      if (fuente === "SEMAFOROS" || fuente === "SEMAFOROSNO"  || fuente === "SEMAFOROSFALLA") {
        ...
        specificsSemaforos.innerText = 
          `Estado: ${estado} / Shutdown: ${id}\nVideo detectores en la intersección: ${tipoIncidente}\nVideo detectores en el corredor: ${barrio} `;
        detailsDiv.appendChild(specificsSemaforos);
      }
    
      if (["WAZE", "IRREGULARIDADES", "CONGESTIONPRIORIDAD", , "CONGESTIONPRIORIDADSEMAFOROS", "STREAMING", "STREAMINGSEMAFOROS", "PMT"].includes(fuente)) {
      ...
      if (fuente === "P1") {
        ...
        verComentarioBtn.onclick = () => {
          ...
          popupContent.textContent = comentarios || "No hay comentarios disponibles";
      ...
      if (["P1", "LLUVIAS", "SEMAFOROS", "SEMAFOROSNO", "SEMAFOROSFALLA", "INTELIGENTES"].includes(fuente)) {
      ...
      if (["P1", "WAZE", "IRREGULARIDADES", "CONGESTIONPRIORIDAD", "CONGESTIONPRIORIDADSEMAFOROS", "STREAMING", "STREAMINGSEMAFOROS", "PMT", "LLUVIAS", "SEMAFOROS", "SEMAFOROSNO", "SEMAFOROSFALLA" ].includes(fuente)) {
        appendFeature(iconUrlClock, "Hora", horaFinal);
        appendFeature(iconUrlCalendar, "Fecha", dia);
      }

      appendFeature(iconCoordenadas, "Coordenadas", `${latitud.toFixed(6)}, ${longitud.toFixed(6)}`);
    ...
    const fetchDataActualizar = async () => {      
      ...
      {    
          ...
          if (currentCenter)
          {
            ...
            onChangeActualizar(true, currentLat ?? 0, currentLng ?? 0, currentZoom ?? 10);
          }
  ...
  return (
    <div className="map-container">
      <div style={{ position: 'absolute', top: '10px', left: '60px', zIndex: 15 }}>
        <input
          id="pac-input"
          className="controls"
          type="text"
