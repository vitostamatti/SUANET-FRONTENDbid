...
export default function NavBarMap({ lat, lng, vistaTrafico}: navBarMapsProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {      
      try {
        ...
        const responseCountLastDay = await fetch(`${backendUrl}/api/countLastHour`);
        ...
        const resTiempoRTAhistorico = await fetch(`${backendUrl}/api/AVGtimeRTAhistoricDay`); 
 ...
 }, [incidentesCheckbox, prioridadCheckbox, inteligentesCheckbox, wazeCheckbox, pmtCheckbox, desviosCheckbox, irregularidadesCheckbox, salvavidasCheckbox, camCGTCheckbox, semaforosCheckbox, streamingCheckbox, lluviasCheckbox, lat, lng, vistaTrafico]);
  ...
  const fetchDataPMTtramos = async () => {
    try
    {
      ...
      if (responseTramoPMT.ok) 
      {
        ...
        if(newDataTramoPMT)      
        {
          const combinedDataTramoPMT = newDataTramoPMT.map((feature: any) => {

            const type = feature.properties.TAFE;
            const address = feature.properties.DINI + " - " + feature.properties.DFIN;
            ...
            {
              const coordinates = feature.geometry.coordinates.map((point: number[]) => [point[0], point[1]]);
              
              return { coordinates, address, type, localidad, company, day, hour };
            }                  
      ...
      //const responseDesvios = await fetch(`/api/desvios-1-reg`);
        if (responseDesvios.ok) {
          ...
          if(newDataDesvios)      
          {
            ...
            const combinedDataDesvios = newDataDesvios.map((feature: any) => {
  ...
  const fetchDataAlertasInteligentes = async () => {
    try
    {
      ...
      const responseInteligentes = await fetch(`${backendUrl}/api/alertas_inteligentes`);
        if (responseInteligentes.ok) 
        {
          ...
          if(inteligentesInfo)      
          {
              const combinedDataInteligentes = inteligentesInfo.map((feature: any) => {
                ...
                const timestamp_startTime = feature.properties.timestamp_startTime;
                ...
                const cantidad_alertas_waze = feature.properties.cantidad_alertas_waze;
                const tipos_alertas_waze = feature.properties.tipos_alertas_waze_unidos;
                ...
                const tiempo_en_congestion = feature.properties.tiempo_en_congestion;
                ...
                return { coordinates, id, acumuladodia_clima, afectados, estado_mvi, length, nivel_alerta, timestamp_startTime, timestamp_endTime, trend, valorlectura_clima, estado_semaforo, tipo_congestion, cantidad_alertas_waze, tipos_alertas_waze, id_incidente, tipo_incidente, indice_congestion, tiempo_en_congestion };
        ...
        {
          setAlertasInteligentes(0);
          console.warn('⚠️ Error al obtener Alertas Inteligentes (endpoint no disponible)');
          return []; // Retornar array vacío para evitar errores de renderizado
        }
    }
    catch (error)
  ...
  const fetchDataInteligentesPoints = async () => {    
    ...
    {   
      ...
      if (responseInteligentes.ok) 
      {
        ...
        {
          console.log('Datos de puntos alertas inteligentes cargados correctamente');

          const combinedDataInteligentes = newDataInteligentes.map((item : any) => {
            const idInteligente = item.properties.uuid;

            const coordinates = item.geometry.coordinates.map((point: number[]) => [point[0], point[1]]);
            ...
            return [latInteligente, lngInteligente, null, null, null, null, null, null, idInteligente, null, null, fuente9];
          });
      ...
      {
        console.warn('⚠️ Error al obtener puntos de alertas inteligentes (endpoint no disponible)');
      }
    }
    catch (error)
    {
  ...
  const fetchDataIndiceCongestion = async () => {
    try
    {
      const responseCongestionVel = await fetch(`${backendUrl}/api/dim-congestion`);
        if (responseCongestionVel.ok) {
          ...
          {
            const combinedDataCongestionVel = newDataCongestionVel.map((properties: any) => {
            ...
            const congestionIndiceAlta = combinedDataCongestionVel.filter((datos: any) => datos.id < 150);            
            setCongestionDataAlta(congestionIndiceAlta);

            const congestionIndiceMedia =  combinedDataCongestionVel.filter((datos: any) => datos.id >=150 && datos.id <= 500)            
            setCongestionDataMedia(congestionIndiceMedia);

            const congestionIndiceBaja =  combinedDataCongestionVel.filter((datos: any) => datos.id > 500)
  ...
  const fetchDataIndiceCongestionAlertas = async () => {
    ...
    try
    {
      ...
      const responseCongestionAlerta = await fetch(`${backendUrl}/api/dim-congestion-first-decil-mapa`);
        ...
        if (responseCongestionAlerta.ok) {
            ...
            if(newDataCongestionAlerta.length > 0)
            {
                combinedDataCongestion = newDataCongestionAlerta.map((data: any) => {
                  ...
                  const [id,address,indicador,calificacion,velocidad,fecha,hora,lat,lng,tipo] = data;

                  const horaArray = hora.split(":");
                  const hour = horaArray[0] + ":" + horaArray[1];

                  return [lat, lng, id, address, null, null, velocidad, calificacion, indicador, fecha, hour, fuente4];
        ...
        {
            console.warn('⚠️ Error al obtener datos de índice de congestión (endpoint no disponible)');
            // No retornamos - continuamos para intentar cargar semáforos
        }
    ...
    try
    {
      ...
      const responseSemaforoAlerta = await fetch(`${backendUrl}/api/falla_semaforos`);
        ...
        {
            console.warn('⚠️ Error al obtener datos de semáforos en falla (endpoint no disponible)');
            // No retornamos - continuamos con los datos que tengamos
        }
    ...
    else if(alertaCongestion && !alertaSemaforos)
    {
      ...
      console.log('✅ Datos de prioridad cargados: solo congestión (semáforos no disponibles)');
    ...
    else if(!alertaCongestion && alertaSemaforos)
    {
      ...
      console.log('✅ Datos de prioridad cargados: solo semáforos (congestión no disponible)');
      return combinedDataSemaforos;
    }
    else
    {
      setAlertasPrioridad(0); // Solo desactivar si ambos fallan
      console.warn('⚠️ No hay datos de prioridad disponibles (ni congestión ni semáforos)');
      return [];
    }
          ...
          if(newDataWazeVel)
          {
            const combinedDataWazeVel = newDataWazeVel.map((feature: any) => {
              ...
              if (geometryWKT && geometryWKT.startsWith("MULTILINESTRING")) {
                ...
                const coordinates = coordinatesString.split(", ").map((coord: string) => {
                  const [lng, lat] = coord.split(" ").map(Number);
                  return [lng, lat];
                });

                return { coordinates, address, hour, day, id, velocity, level, delay };
              }
  ...
  const fetchDataSemaforos = async () => {    
    ...
    {   
      ...
      const responseSemaforos = await fetch(`${backendUrl}/api/estados_semaforizacion`);
      ...
      if (responseSemaforos.ok) 
      {
        ...
        if(newDataSemaforos.length > 0)
        {
          ...
          const combinedDataSemaforos = newDataSemaforos.map((item : any) => {
            ...
            return [latSemaforos, lngSemaforos, estado, dirSemaforos, videoDetectorInterseccion, videoDetectorCorredor, localidad, descripcion, idSemaforos, day, horaCompleta, fuente5];
  ...
  const fetchDataStreaming = async () => {    
    ...
    {   
      ...
      const responseStreaming = await fetch(`${backendUrl}/api/camaras_streaming`); 
      ...
      if (responseStreaming.ok) 
      {
        ...
        if(newDataStreaming.length > 0)
        {
          ...
          const combinedDataStreaming = newDataStreaming.map((item : any) => {
            ...
            return [latStreaming, lngStreaming, null, dirStreaming, null, null, null, camaraActiva, idStreaming, day, horaCompleta, fuente6];
          });
  ...
  const fetchDataStreamingSemaforos = async () => {    
    ...
    {   
      ...
      const responseStreaming = await fetch(`${backendUrl}/api/camaras_semaforos`); 
      ...
      if (responseStreaming.ok) 
      {
        ...
        if(newDataStreaming.length > 0)
        {
          ...
          const combinedDataStreamingSemaforos = newDataStreaming.map((item : any) => {
            ...
            return [latStreamingSemaforos, lngStreamingSemaforos, null, dirStreamingSemaforos, null, null, null, funcionamientoSemaforos, idStreamingSemaforos, day, horaCompleta, fuente6, idCruceSemaforos, nomenclaturaCruceSemaforos, rotacionSemaforos];
          });
      ...
      if (responseCamCGT.ok) 
      {
        ...
        if(newDataCamCGT.length > 0)
        {
          ...
          const combinedDataCamCG = newDataCamCGT.map((item : any) => {
            ...
            return [latCamCGT, lngCamCGT, null, null, null, null, null, null, idCamCGT, null, null, fuente4];
  ...
  const fetchDataSalvavidas = async () => {    
    ...
    {   
      ...
      const responseCamSalvavidas = await fetch(`${backendUrl}/api/camaras-salvavidas`);
      ...
      if (responseCamSalvavidas.ok) 
      {
        ...
        if(newDataCamSalvavidas.length > 0)
        {
          ...
          const combinedDataCamSalvavidas = newDataCamSalvavidas.map((item : any) => {
            const idCamSalvavidas = item.camaras_salvavidas_id;
            const latCamSalvavidas = item.y;
            const lngCamSalvavidas = item.x;
            
            return [latCamSalvavidas, lngCamSalvavidas, null, null, null, null, null, null, idCamSalvavidas, null, null, fuente3];
  ...
  const fetchDataLluvias = async () => {    
    ...
    {   
      ...
      if (responseLluvias.ok) 
      {
        ...
        if(newDataLluvias.length > 0)
        {
          ...
          const combinedDataLluvias = newDataLluvias.map((item : any) => {
            ...
            const descripcionLluvias = item.TIPOTECNOLOGIA + " Altitud " + item.ALTITUD;
            ...
            const horaCompletaLluvias = horaArrayLluvias[0] + ":" + horaArrayLluvias[1];

            return [latLluvias, lngLluvias, statusLLuvias, dirLLuvias, tipoLluvias, valorlecturaLluvias, localidadLLuvias, descripcionLluvias, idLluvias, diaLluvias, horaCompletaLluvias, fuente8];
      ...
      {
        console.error(`❌ [Rain Data] HTTP Error: ${responseLluvias.status} - ${responseLluvias.statusText}`);
    ...
    catch (error) 
    {
        ...
        if (error instanceof TypeError && error.message.includes('fetch')) {
          console.error('❌ [Rain Data] This appears to be a network connectivity issue');
        ...
        console.warn('⚠️ [Rain Data] Rain data is temporarily unavailable. This may be due to:');
    ...
    { 
      ...
      if (responsePMT.ok) 
      {
        ...
        if(newDataPMT)
        {
          ...
          const combinedDataPMT = pmtInfo.map((feature: any) => {
            ...
            if (latitud && longitud) 
            {              
              return [latitud, longitud, null, address, type, null, null, subtype, id, day, hour, fuente7];
            }
    ...
    {   
      ...
      if (responseWaze.ok) 
      {
        ...
        if(newDataWaze)
        {
          ...
          const combinedDataWaze = newDataWaze.map((feature: any) => {
            ...
            if ( latitud && longitud) {
              return [latitud, longitud, null, address, type, null, null, subtype, id, day, hour, fuente2];
            }
  ...
  const fetchDataIrregularidades = async () => {    
    ...
    {   
      ...
      const responseIrregularidades = await fetch(`${backendUrl}/api/dim-irregularities`);
      if (responseIrregularidades.ok) 
      {
        ...
        {
          console.log('Datos de alertas de irregularidades cargados correctamente');

          const combinedDataIrregularidades = newDataIrregularidades.map((feature: any) => {
            ...
            if ( latitud && longitud) {
              return [latitud, longitud, null, address, type, null, null, subtype, id, day, hour, fuente3];
            }
  ...
  const handleActualizar = (on: boolean, lati: number, longi: number, zoom: number) => {
  ...
  return (
    <div className="w-full h-screen px-0 py-0">
...

      {
        ...
        <Map ubicaciones={dataGeo} velocidadesWaze1={wazeDataVel1} velocidadesWaze2={wazeDataVel2} velocidadesWaze3={wazeDataVel3} velocidadesWaze4={wazeDataVel4} velocidadesWaze5={wazeDataVel5} tramoPMT={pmtTramoData} desvios={desviosData} inteligentes={inteligentesData} opcDropdownVel={opcVel} lat={latitud} lng={longitud} congestionDataAlta={congestionDataAlta} congestionDataMedia={congestionDataMedia} congestionDataBaja={congestionDataBaja} ubicacionesAlertas={dataGeoAlertas} onChangeActualizar={handleActualizar} onChangeActualizarStreaming={handleActualizarStreaming} onChangeOrigins={handleOriginsChange} onChangeRecomendaciones={handleRecomendacionesChange} zoom={zoom} recomendaciones={alertasRecomendaciones} areaslocalidades={areasLocalidades} />
      }
