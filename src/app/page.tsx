...
import { CoordinateProvider, useCoordinates } from '../../components/coordinateContext';
...
function UnauthorizedUserMessage({ email, onClose }: { email: string; onClose: () => void }) {
  return (
      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            El usuario con correo <strong>{email}</strong> no está autorizado para acceder al sistema SUANET.
          </p>
...
function AuthErrorMessage({ error, onClose }: { error: string; onClose: () => void }) {
    ...
    switch (errorType) {
      ...
      case 'no_user':
        return {
          ...
          message: 'No se pudo obtener la información de tu cuenta. Por favor, intenta nuevamente.'
        };
      case 'no_token':
        return {
          title: 'Error de Sesión',
          message: 'No se pudo establecer la sesión. Por favor, intenta nuevamente.'
        };
      case 'server_error':
        return {
...
function HomeContent() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!registrationStatus) {
        ...
        if (!result.success) {
          console.error('❌ No se pudo registrar frontend, OAuth podría fallar');
          alert('Error de configuración. Por favor, intenta nuevamente en unos momentos.');
          return;
        }
        
        console.log('✅ Frontend registrado exitosamente, procediendo con OAuth');
      }
    ...
    return (
      <div className="fixed inset-0 min-h-screen w-full flex items-center justify-center bg-gray-50 z-50">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-2">Configurando conexión con servidor...</p>
        </div>
      </div>
    );
  }
  ...
  if (unauthorizedEmail) {
    return <UnauthorizedUserMessage email={unauthorizedEmail} onClose={handleCloseError} />;
  }
