...
const applyBasicSecurityHeaders = (req, res, isAPICall = false) => {
  ...
  if (req.url && (req.url.includes('/video_feed/') || req.url.includes('/video_detector_feed/'))) {
    console.log(`🎥 [Video CORS] Setting CORS headers for video stream: ${req.url}`);
...
// ============================================
let dynamicBackendUrl = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_BACKEND_URL;
...
const updateBackendUrl = (newBackendUrl) => {
  ...
  console.log(`🔄 [Frontend Server] Backend URL updated: ${oldUrl} -> ${newBackendUrl}`);
...
const registerWithBackend = async () => {
  try {
    ...
    console.log(`🌐 [Frontend Server] Registrando frontend ${frontendUrl} con backend ${dynamicBackendUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`❌ [Frontend Server] Response statusText: ${response.statusText}`);
    ...
    console.warn('⚠️ [Frontend Server] Error registrando con backend (non-blocking):', error.message);
...
app.prepare().then(() => {
  const server = createServer((req, res) => {
      ...
      req.on('end', async () => {
        try {
          ...
          if (!backendUrl) {
            ...
            res.end(JSON.stringify({ success: false, message: 'backendUrl requerido' }));
            return;
          }
          ...
          console.log(`🔄 [Frontend Server] Backend URL actualizada: ${oldUrl} -> ${backendUrl}`);
    ...
    if (pathname.startsWith('/api')) {
      console.log(`🔀 [Frontend Server] Proxying ${pathname} -> ${getBackendUrl()}`);
        ...
        proxy.web(req, res, {
          ...
          onProxyRes: function (proxyRes, req, res) {
            ...
            proxyRes.on('data', (chunk) => {
              if (responseBody.length < 500) { // Only log first 500 chars
                responseBody += chunk.toString();
              }
            });
            
            proxyRes.on('end', () => {
              console.log(`📋 [Process Stream] Response preview (first 500 chars):`);
              console.log(responseBody.substring(0, 500));
              
              if (contentType.includes('application/json')) {
                console.log(`✅ [Process Stream] Response is JSON (expected)`);
              } else if (contentType.includes('image/')) {
                console.log(`⚠️ [Process Stream] Response is IMAGE (unexpected - should be JSON)`);
              } else {
                console.log(`❓ [Process Stream] Response type unclear: ${contentType}`);
              }
            });
          },
      ...
      } else if (pathname.includes('/video_feed/') || pathname.includes('/video_detector_feed/')) {
        // Special handling for video streams to preserve MJPEG headers
        console.log(`📹 [Video Stream] Proxying ${pathname} to remote backend ${getBackendUrl()}`);
    ...
    if (pathname === '/reflectividad.kmz') {
      try {
        ...
        if (!normalizedPath.startsWith(kmzDir)) {
          console.error('[KMZ Server] Path traversal attempt detected:', pathname);
          res.writeHead(403);
          res.end('Forbidden');
          return;
        }
        ...
        try {
          ...
          res.setHeader('Content-Disposition', 'attachment; filename="reflectividad.kmz"');
    ...
    if (pathname === '/ws-test') {
      res.writeHead(302, { 'Location': '/videoanalitica/websocket-diagnostics.html' });
      res.end();
      return;
    }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (pathname.startsWith('/socket.io')) {
      console.log('Solicitud WebSocket Socket.IO recibida, redirigiendo al backend');
  ...
  function downloadKmzFile() {
    const sourceUrl = 'https://evp.sire.gov.co/radar_animate/datos/kml/reflectividad.kmz';
    ...
    const expectedDir = path.normalize(path.join(process.cwd(), 'public', 'kmz'));
    ...
    try {
      ...
      try {
        ...
        console.error(`[KMZ Updater] Error al crear directorio ${normalizedDestDir}: ${err.message}`);
        return;
      }
    }
    ...
    if (!normalizedDestPath.startsWith(expectedDir) || !tempFilePath.startsWith(expectedDir)) {
      console.error('[KMZ Updater] Invalid file path detected');
      return;
    }
    
    console.log(`[KMZ Updater] Iniciando descarga desde ${sourceUrl} a ${destinationPath}`);
    console.log(`[KMZ Updater] Network info: NODE_ENV=${process.env.NODE_ENV}, timestamp=${new Date().toISOString()}`);
    
    const request = https.get(sourceUrl, (response) => {
      console.log(`[KMZ Updater] ✅ Conexión establecida con ${sourceUrl}`);
      // Verificar si la respuesta es exitosa
      if (response.statusCode !== 200) {
        console.error(`[KMZ Updater] Error al descargar archivo. Código de estado: ${response.statusCode}`);
        return;
      }
          ...
          if (err) {
            ...
            try {
              ...
              if (cleanupErr.code !== 'ENOENT') {
                console.error(`[KMZ Updater] Error limpiando archivo temporal: ${cleanupErr}`);
              }
            }
            return;
          }
          
          console.log(`[KMZ Updater] Archivo reflectividad.kmz actualizado exitosamente a las ${new Date().toLocaleTimeString()}`);
        });
      });
    });
    ...
    request.setTimeout(30000, () => {
      request.abort();
      console.error(`[KMZ Updater] ⏰ Timeout después de 30 segundos intentando conectar a ${sourceUrl}`);
      // ✅ SEGURIDAD: Limpiar archivo temporal de forma segura
      try {
        fs.accessSync(tempFilePath, fs.constants.F_OK);
        fs.unlinkSync(tempFilePath);
  ...
  server.listen(port, (err) => {
    ...
    console.log(`> WebSocket test disponible en: http://localhost:${port}/ws-test`);
    ...
    registerWithBackend().then(registrationSuccess => {
      if (registrationSuccess) {
        console.log('✅ [Frontend Server] Sistema de proxy dinámico configurado');
      } else {
        console.warn('⚠️ [Frontend Server] Registro inicial falló, continuando con configuración estática');
      }
    }).catch(error => {
      console.warn('⚠️ [Frontend Server] Registration error (non-blocking):', error.message);
    });
    ...
    setInterval(async () => {
      try {
        console.log('🔄 [Frontend Server] Re-registro periódico (reducido)...');
        await registerWithBackend();
      } catch (error) {
        console.warn('⚠️ [Frontend Server] Periodic registration failed:', error.message);
      }
    }, 15 * 60 * 1000); // ✅ CAMBIADO: Cada 15 minutos en lugar de 5 para reducir conflictos

    console.log(`> Actualizador KMZ iniciado - descargando reflectividad.kmz cada minuto`);
  });
